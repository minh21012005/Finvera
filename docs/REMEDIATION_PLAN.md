...
| 2026-09-07 | **Q-62 fixed.** The second defect behind `889/1522`: `daily_bars_current` demanded a bar dated `--end`, which 603 of 1,522 delisted/untraded symbols can never have, so they were re-fetched on every re-run and the crawl could never report itself complete - the permanent noise that hid Q-61. Coverage now means "fetched for this window" (checkpoint range plus the package's own `rangeEnd`) rather than "traded on `--end`"; the one-day, self-healing same-day-rerun cost is stated in R-009 rather than hidden. Replayed on the real checkpoint: `daily_bars_current` 870 -> 1,473, `is_finished` 889 -> 1,168; suite 94/94 (specs/026 R-009, FR-008/SC-7, T013). |
