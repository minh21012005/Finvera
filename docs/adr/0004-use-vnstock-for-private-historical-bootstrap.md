...
ADR-0003 originally selected TCBS iFlash for live private market data. The
later ADR-0009 pivot removes TCBS from the active runtime path and promotes
Vnstock/KBS to the primary private provider for owner-operated market packages.
TCBS's documented price-history operation covers intraday matches and does not
establish the completed daily OHLCV depth required by `market-regime-v1`,
including SMA200 and a 252-session volatility window.
...
Use a pinned Vnstock release as an offline owner-operated market-data package
tool for the private deployment. Its initial scope was historical bootstrap and
bounded gap recovery; ADR-0009 extends it to primary private market-overview
packages while keeping the same Spring import boundary.
...
- Vnstock does not run in `finvera-ai`, in the browser, or as a permanent
  deployable service.
- TCBS no longer remains the active live source. Overlapping completed-session
  facts from any future provider are retained separately and reconciled;
  material conflict withholds the affected regime.
...
bootstrap/package workflow, no guaranteed realtime stream in Community, and no
SLA or public-use right. Missing or conflicting history degrades the affected
sections rather than fabricating facts.
