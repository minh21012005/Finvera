...
| Local Vnstock exporter before upstream terms are independently verified | The owner accepts residual upstream-use risk solely for manual, loopback-only, non-commercial bootstrap. The exporter is needed to exercise the already-tested canonical import boundary with real historical observations. | Waiting for a formal KBS confirmation blocks private experimentation but does not improve the technical importer | Owner approval, 2026-08-17; contract records the limitation | Revoke immediately for public, remote, multi-user, scheduled, or redistributed use. T047 remains open as the evidence/compliance gate. |
