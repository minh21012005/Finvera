...
- [x] T090 [FR-001, FR-002, FR-011, DATA-004] Keep live session facts inside their trading date and refuse a zero reference price: `TcbsLiveEquityQuoteService` stamps each `s|4` reference with the venue session it arrived in and discards prior-session open/high/low/reference when a trade or read belongs to a later session; `BreadthCalculator` treats a zero official reference like a missing one (`MISSING_REFERENCE_PRICE`) instead of classifying every positive price ADVANCING.
      Verify: `TcbsLiveEquityQuoteServiceTests` (6, incl. the new rollover scenario proving yesterday's reference is not reused) and `BreadthCalculatorTests` (6, incl. the zero-reference case) pass.
      Evidence (2026-08-30): tracked as Q-24 and part of Q-29 in `docs/REMEDIATION_PLAN.md`. `sessionFacts`/`referencePrices` were process-lifetime maps never reset by date, so after midnight the displayed change % and O/H/L were the previous session's until a restart.
