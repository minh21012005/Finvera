...
- [x] T049 [AI-001, FR-003, FR-006] Q-44 (2026-08-31) tool payload audit. `ToolDelegateService`: (A) a withheld valuation was reported as `"FAIR_VALUE"` (not even a real label) → `classification` is null with `published=false` and the engine's `reasonCodes`; (B) `PE_RATIO`/`PB_RATIO` aliases never matched the catalog codes `PE`/`PB` → peRatio/pbRatio were always null; now populated, plus `score`, `displayedScore`, `confidence`, `ruleVersion`, `historyPointCount`, `sectorConstituentCount`, per-metric `MetricFact` (value, applicability, qualityReason, percentiles, effectiveWeight); (C) `REVENUE_GROWTH` alias → `REVENUE_GROWTH_PERCENT`; fundamentals now carry `epsTtm`, `epsGrowthPercent` and `MetricFact`s so `ANNUAL_BASIS`/`TTM` survive; (D) unavailable price is `null` + `dataStatus`/`reasonCodes`, never `"0"`; (E) technical returns every triggered strategy (`signals`) with levels/risk and valued risk factors, `signal` = strongest. `finvera-ai`: tool calls dispatched concurrently (bounded 5) with proposal-order events; synthesis prompt embeds JSON not Python repr; offline templates no longer default a classification. Contract `internal-api.openapi.yaml` amended additively (classification nullable).
      Verify: `ToolDelegateServiceTests` (+4), `InternalToolControllerTests`, `test_t049_tools_dispatch_concurrently_but_events_keep_proposal_order`; backend full suite, AI 88/88.
      Addendum (owner VIC test, 2026-08-31): all 19 stated figures verified against the DB. Two disclosure gaps closed: `StockValuation`/`ValuationTool` now carry `priceTradingDate` (the close that priced the assessment — 28/08 while asOf was the 31/08 session) and `inputBasis` (e.g. `EPS_GROWTH_PERCENT -> ANNUAL_BASIS`, so a PEG built on annual growth is disclosed). FE `groupClaimsBySentence` shows each verified sentence once with all its verified fields instead of once per tag (`claim-grouping.test.ts`).
- [x] T050 [FR-013, AI-003] Answer presentation (owner report 2026-08-31: the VIC answer rendered as one flat line). Cause: `strip_synthesis_tags` collapsed every whitespace run — newlines included — to one space, and the FE printed the answer as plain text. Fix: the stripper keeps line structure (only inner space runs collapse, indentation preserved, `VND .` → `VND.`); FE renders through `format/lite-markdown.tsx` (bold, bullet/numbered lists, nesting by indentation, paragraphs) built from React elements only — model text is never injected as HTML; `format/citation-tags.ts` removes `[T…]`/`[Block n]` tags from the live stream; `format/claim-grouping.ts` shows each verified sentence once with all its verified fields.
      Verify: `test_strip_synthesis_tags_keeps_markdown_line_structure`, `lite-markdown.test.tsx` (incl. HTML-injection guard), `claim-grouping.test.ts`; AI 89/89, FE 138/138 + lint/build, backend 670/670.
