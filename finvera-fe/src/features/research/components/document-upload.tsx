...
export function DocumentUpload({ onDocumentSubmitted }: DocumentUploadProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Use existing idempotencyKey across retries
      const result = await submitDocument(params, idempotencyKey);
      onDocumentSubmitted(result);

      // Reset form and generate a fresh key for next genuine submission
      setTitle("");
      setSymbol("");
      setQuarter("");
      setFile(null);
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm">
      <h3 className="text-lg font-semibold text-slate-100 mb-4 flex items-center justify-between">
        <span>Tải lên Tài liệu Nghiên cứu</span>
        <div className="inline-flex rounded-lg bg-slate-800 p-1 text-xs">
          <button
            type="button"
            className={`px-3 py-1 rounded-md transition-colors ${
              uploadMode === "file" ? "bg-cyan-600 text-white font-medium" : "text-slate-400 hover:text-slate-200"
            }`}
          ...
          <button
            type="button"
            className={`px-3 py-1 rounded-md transition-colors ${
              uploadMode === "text" ? "bg-cyan-600 text-white font-medium" : "text-slate-400 hover:text-slate-200"
            }`}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Mã Cổ phiếu (Tùy chọn)</label>
            <input
              type="text"
              className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 uppercase focus:outline-none focus:border-cyan-500"
              placeholder="FPT"
              value={symbol}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">Năm</label>
              <input
                type="number"
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500"
                value={year}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">Quý</label>
              <select
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500"
                value={quarter}
        ...
        {uploadMode === "file" ? (
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">
              File PDF <span className="text-rose-400">*</span>
            </label>
            <input
              type="file"
              accept=".pdf,application/pdf"
              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-sm text-slate-300 file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-slate-800 file:text-slate-200 hover:file:bg-slate-700"
        ...
        <div className="flex justify-end pt-2">
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-2 bg-cyan-600 hover:bg-cyan-500 text-white font-medium text-sm rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? "Đang xử lý..." : "Tải lên & Bắt đầu Xử lý"}
          </button>
        </div>
      </form>
    </div>
