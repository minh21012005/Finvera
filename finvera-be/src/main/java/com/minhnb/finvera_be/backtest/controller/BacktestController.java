...
public class BacktestController {
    ...
    @GetMapping("/{id}/trades") public Page<Trade> trades(@PathVariable UUID id,@RequestParam(defaultValue="100") @Min(1) @Max(5000) int limit,@RequestParam(defaultValue="0") @Min(0) int offset){return queries.trades(id,limit,offset);}
    @GetMapping("/{id}/equity") public Page<EquityPoint> equity(@PathVariable UUID id,@RequestParam(defaultValue="100") @Min(1) @Max(10000) int limit,@RequestParam(defaultValue="0") @Min(0) int offset){return queries.equity(id,limit,offset);}
    @GetMapping("/{id}/events") public Page<EntryEvent> events(@PathVariable UUID id,@RequestParam(defaultValue="100") @Min(1) @Max(5000) int limit,@RequestParam(defaultValue="0") @Min(0) int offset){return queries.events(id,limit,offset);}
    @DeleteMapping("/{id}") public ResponseEntity<Void> delete(@PathVariable UUID id){commands.delete(id);return ResponseEntity.noContent().build();}
    @DeleteMapping public ResponseEntity<Void> deleteAll(){commands.deleteAll();return ResponseEntity.noContent().build();}
